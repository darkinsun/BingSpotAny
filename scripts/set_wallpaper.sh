...
# Bug fixes are automatically applied by Variety **only if** the local copy was never changed by the user. If there
# were user-made changes to it, Variety would assume the existing script works for the user and would not overwrite it
# on upgrade.
#
# PARAMETERS:
...
wlroots_set() {
    if pgrep wpaperd --quiet; then
        wpaperctl set "$WP"
    elif pgrep awww-daemon --quiet; then
        awww img "$WP"
...
if [ "$DE" == "enlightenment" ]; then
    ...
    # If wallpaper changing tool is available, use it as prevents system freeze.
    ...
    # Detect wallpaper changing program
    if ! wlroots_set; then
        if pgrep hyprpaper --quiet; then
            MONITOR_IDS=($(hyprctl monitors all -j | jq '.[].name' -r))
            # set bg for all monitors
            BG_EXIT=0
            for id in "${MONITOR_IDS[@]}"; do
                (hyprctl hyprpaper wallpaper "$id","$WP")
                BG_EXIT=$?
            done
            # check if background change has failed
            if [ $BG_EXIT != 0 ]; then
                hyprctl notify 3 10000 0 "Variety could not communicate with hyprpaper, even though it is running. Is IPC of hyprpaper active?"
            fi
        else
            hyprctl notify 3 10000 0 "Variety could not detect a program to change the background"
        fi
    fi
